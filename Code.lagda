...
\begin{code}
  _⊚_ : (A : H↑) (P : (s : 𝒮 Σ) → H↑) → H↑
  (A ⊚ P) (Υ ∥ Γ) = ⨛[ H ∋ h ] let Υ′ ∥ Δ = h in A (Υ′ ∥ Δ) ⊗ ⨜[ Sym ∣ Υ′ ∣ ∋ u ] Y (Υ′ [ u ]) (Υ ∥ Γ) ⊗ ⨜[ Var ∣ Δ ∣ ∋ x ] P (Δ [ x ]) (Υ ∥ Γ)
\end{code}
%</tensor0>
...
\begin{code}
    _♯
      ...
      → (∀ {s} (D : P s (Υ ∥ Δ)) → P s (Υ ∥ Γ))
    f ♯ = ς ∘ s↑ ∘ ⟨ id , ![ (λ u → (u , refl) , f ∘Π (_, refl)) ] ⟩

    ⟨_,_⟩♯
      : ∀ {Υ Υ′ Δ Γ}
      → (ρ : ∀ {s} (x : Y s (Υ′ ∥ Δ)) → Y s (Υ ∥ Γ))
      → (f : ∀ {s} (x : V s (Υ′ ∥ Δ)) → P s (Υ ∥ Γ))
      → (∀ {s} (D : P s (Υ′ ∥ Δ)) → P s (Υ ∥ Γ))
    ⟨ ρ , f ⟩♯ = ς ∘ s↑ ∘ ⟨ id , ![ (λ u → ρ (u , refl) , f ∘Π (_, refl)) ] ⟩
\end{code}
%</extension>
