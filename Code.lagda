...
\begin{code}
    ...
    S^-concat : ∀ {Γ Δ} → (S^[ Γ ] ⊗↑ S^[ Δ ]) ~> S^[ Γ ,, Δ ]
    S^-concat {Γ = Γ} {Δ = Δ} (⟦Γ⟧ , ⟦Δ⟧) i with fin+-split (∣ Γ ∣) ∣ Δ ∣ i
    S^-concat {Γ} {Δ} (⟦Γ⟧ , ⟦Δ⟧) .(fin+-inl i) | fin+-left i = ⟦Γ⟧ i
    S^-concat {Γ} {Δ} (⟦Γ⟧ , ⟦Δ⟧) .(fin+-inr {∣ Γ ∣} j) | fin+-right j = ⟦Δ⟧ j

    ^-concat : ∀ {Γ Δ X} → (X ^[ Γ ] ⊗↑ X ^[ Δ ]) ~> X ^[ Γ ,, Δ ]
    ^-concat {Γ = Γ} {Δ = Δ} (⟦Γ⟧ , ⟦Δ⟧) i with fin+-split (∣ Γ ∣) (∣ Δ ∣) i
    ^-concat {Γ} {Δ} {X} (⟦Γ⟧ , ⟦Δ⟧) .(fin+-inl i) | fin+-left i = ⟦Γ⟧ i
    ^-concat {Γ} {Δ} {X} (⟦Γ⟧ , ⟦Δ⟧) .(fin+-inr {∣ Γ ∣} j) | fin+-right j = ⟦Δ⟧ j
\end{code}
%</interpretation>
