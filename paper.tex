...
\begin{remark}
  ...
  also forms a \emph{relative monad} on the functor
  $\Of{V}{\widehat{\HypCat}^\Sorts}$ of variables.\footnote{Relative monads are
    skew-monoids in the skew-monoidal structure induced by a functor category
    ...
    functor.} Consequently, the monoid multiplication $\odot$ can be replaced
  with an extension operation:
  \[
    \begin{tikzcd}[ampersand replacement = \&]
      {\IMode{P(\,=\,)^{V(\,-\,)}}} \arrow[r, "(\,-\,)^{P}"]
      \& {\IMode{P(\,=\,)^{P(\,-\,)}}}
