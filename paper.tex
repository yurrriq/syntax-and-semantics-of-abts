...
variable contexts, and $\Define{\Seq}{\SCtx\times\Ctx\times\Sorts}$ is the category of
sequents $\Upsilon\parallel\Gamma\vdash s$.  Then, we fix the presheaf category $\Sets^\Seq$ as
our semantic universe.

\subsection{Substitution monoidal structures}

Any two presheaves $A,B : \Sets^\Seq$ have a tensor $A\bullet B$,
defined as a coend in the following way:
\[
  \Define{(A\bullet B)(\Upsilon\parallel\Gamma\vdash s)}{
    \int^{\Delta\in\Ctx}
      A(\Upsilon\parallel\Delta\vdash s)
        \otimes \prod_{x\in\Dom\Delta} B(\Upsilon\parallel\Gamma\vdash\Delta(x))
  }
\]

The presheaf of variables $\Define{V(\Upsilon\parallel\Gamma\vdash s)}{\MkSet{x \in \Dom\Gamma
\mid \Gamma(x) = s}}$ is the unit to this tensor, and together they comprise a
\emph{substitution monoidal structure}. Then, a presheaf $A$ is a monoid if it is
equipped with the following natural transformations:
...
Intuitively, $\nu$ embeds variables into $A$ and $\varsigma$ equips $A$ with an
operation for simultaneous substitutions of variables.

\begin{remark}
  A \emph{relative monad} on a functor $\Of{J}{\mathbf{J}\to\mathbf{C}}$ is a
  monoid in the functor category $\mathbf{C}^\mathbf{J}$
  \cite{altenkirch-chapman-uustalu:2015}. A presheaf $A$ with a substitution
  monoidal structure $\Pair{\nu}{\varsigma}$ can be viewed as a relative monad
  on the presheaf $\Of{V}{\Sets^\Seq}$ of variables.
\end{remark}
...
endofunctor $\Of{\SigFun{\Sigma}}{\Sets^\Seq\to\Sets^\Seq}$, which is defined
as follows:

\begin{align*}
  \IMode{\SigFun{\Sigma}(X)(\Upsilon\parallel\Gamma\vdash s)}
    &\triangleq
    \OMode{
      \coprod_{\vartheta\in\Operators\Pair{\Upsilon}{\MkArity{\vec{v}}{s}}}
      \prod_{\MkValence{\vec{p}}{\vec{q}}{s'}\in\vec{v}}
      X(\Upsilon,\vec{p}\parallel\Gamma,\vec{q}\vdash s')
    }
\end{align*}
...
Then, a $\Sigma$-model is a monoid $A$ which is equipped with an initial
algebra $\Of{\alpha}{\SigFun{\Sigma}(A)\to A}$, which shall interpret
...
\ExecuteMetaData[latex/Code.tex]{fin}

\subsection{Name}

\ExecuteMetaData[latex/Code.tex]{name}
