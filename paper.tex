...
\begin{remark}
  ...
  discussing covariant sheaves on the category of symbol contexts $\SCtx$, as
  opposed to covariant sheaves on finite sets of symbols $\SymSets$ (i.e.\
  unisorted contexts).
\end{remark}
...
$\Of{\Operators}{\Sheaves{\OpCat{\left(\SCtx\right)}\times\Discrete{\Arities}}}$
such that the arrows in $\SCtx$ lift to renamings of operators' parameters.
Together, $\Sorts$ and $\Operators$ form a \emph{signature}
...
Let $\FinSets$ be category of finite cardinals and \emph{any} functions between
them; then the comma construction $\Ctx$ is the category of variable contexts.
