...
\newcommand\NotSubset[2]{\IMode{#1}\not\subst\IMode{#2}}

\newcommand\SortExp{\mathsf{exp}}

\newcommand\SubstIs[4]{%
  \left[\IMode{#1} / \IMode{#2}\right]\IMode{#3}%
  \leadsto\OMode{#4}%
}
...
cocartesian category over $\Symbols\times\Sorts$. Then, fix a covariant
presheaf of operators $\Operators:\Finsets\times\Arities\to\Sets$ such that the
arrows in $\Finsets$ lift to renamings of operators' parameters; via the
Grothendieck construction, we can also consider the set $\Grothendieck\Operators$ of
operators $(\Upsilon,\varrho,\vartheta)$ for
...
pair of judgments, $\SubstIs{N}{x}{M}{M'}$ and $\SubstIs{N}{x}{E}{E'}$:
