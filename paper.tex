...
Then, the variables object
$\Define{V_s(\Upsilon\parallel\Gamma)}{\MkSet{x\in\Dom\Gamma\mid \Gamma(x)=s}}$
is the unit to this tensor; additionally, we have a presheaf of symbols
$\Define{Y_s(\Upsilon\parallel\Gamma)}{\MkSet{u\in\Dom\Upsilon\mid
\Upsilon(x)=s}}$. We will say that an object
...
$Y^{\{\Upsilon\}}$ shall mean $\prod_{u\in\Dom\Upsilon}Y_{\Upsilon(u)}$.
Additionally, we have an induced map $\Of{\nu_\Gamma}{V^{[\Gamma]}\to
P^{[\Gamma]}}$.
...
We define the interpretation of a term $M$ in the model $P$ as a morphism
...
$\SemBrackets{\Omega\triangleright\Upsilon\parallel\Gamma\vdash x_j:s}_P$ is
...
$\Lookup{\Omega}{\MV{m}}{\MkValence{\vec{p}}{\vec{q}}{s}}$, is the following composite
\[
  \begin{tikzcd}[ampersand replacement = \&, sep=large]
    {\IMode{\SemBrackets{\Omega\triangleright\Upsilon\parallel\Gamma}_P}} \arrow[r, "\IMode{\Tuple{\pi_i\,\pi_1, \phi}}"]
    \& {\IMode{P_{s'}^{\Yoneda{\vec{p}\parallel\vec{q}}} \times P^{[\vec{q}]}}} \arrow[r, "\IMode{\varsigma_{\vec{q}}}"]
    \& \IMode{P_s}
  \end{tikzcd}
  \tag{metavar}
\]
