...
cover. A sieve $S\sqsubseteq\Yoneda{C}$ is non-empty when the collection
$\bigcup_D S(D)$ of all its arrows is non-empty.
...
\begin{quote}
  A presheaf $X:\Sets^\SCtx$ is a sheaf on $\SCtxSite$ just when, for all $\Member{n}{X(\Upsilon')}$, if we have
  \begin{tikzcd}[cramped]
    \IMode{\Upsilon}\arrow[r, hook, "\IMode{\varrho}"] & \IMode{\Upsilon'}
  \end{tikzcd}
