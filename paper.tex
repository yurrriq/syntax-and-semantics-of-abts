...
\newcommand\Sheaves[1]{\mathbf{Sh}(#1)}
...
the the comma construction
$\Define{\SCtx}{\Comma{\SymSets}{\Discrete{\Sorts}}}$, with $\Discrete{\Sorts}$
...
Now, a covariant presheaf $X$ on $\SCtx$ is an intensional set which is subject to
renamings
\begin{tikzcd}[cramped]
...
$X$, it is precisely the glueing condition for covariant sheaves on $\SCtx$
under the atomic topology. The sheaf condition, then, ensures that the notion
of support is well-behaved.
...
\begin{remark}
  ...
  and, equivalently, Pitts' category of nominal sets; we differ only in
  discussing sheaves on the category of symbol contexts $\SCtx$, as opposed to
  sheaves on finite sets of symbols $\SymSets$ (i.e.\ unisorted contexts).
\end{remark}

\subsection{Operators and signatures}

Fix a sheaf of operators
$\Of{\Operators}{\Sheaves{\OpCat{\SCtx}\times\Discrete{\Arities}}}$ such that the arrows
...
in which they reside, such that the glueing condition is satisfied:
\begin{alignat*}{3}
  \null
&
...
Abts are identified up to $\alpha$-equivalence. Let $\FV{M}$ be the collection
of variables free in $M$, which is easily calculated by recursion on the
structure of $M$. We also intend to calculate the collection $\FS{M}$ of
symbols free in $M$, but this is more involved.
