...
\newcommand\Renaming[2]{
  #2\upharpoonright #1
}
\newcommand\IsRenaming[3]{
  \Renaming{\IMode{#1}}{\IMode{#2}}%
  \leadsto\OMode{#3}%
}
...
$\Of{\varrho}{\Upsilon\to\Upsilon'}$ and $\Of{f}{a\to a'}$, a restriction map
$\Restriction{\Pair{\varrho}{f}}$ such that
$\IsOperator{\Upsilon'}{\Restriction{\Pair{\varrho}{f}}(\vartheta)}{a}$ when
$\IsOperator{\Upsilon}{\vartheta}{a'}$. Because the category
$\Discrete{\Arities}$ is discrete and has only identity arrows, we will often
write $\Restriction{\varrho}$ to mean $\Restriction{\Pair{\varrho}{\ArrId}}$
through an abuse of notation. In particular, the renaming
...
\begin{gather*}
  ...
  \infer{
    ...
    \Match{\Restriction{\varrho}(\vartheta)}{\vartheta'} &
    \HypJ{
...
and so we are justified in writing $\Renaming{\varrho}{M}$ for $N$ when
