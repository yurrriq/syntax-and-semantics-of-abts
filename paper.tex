...
\begin{alignat*}{3}
  &\IMode{{\pi_{\Operators}}^{-1}{\Pair{\Upsilon}{a}}}&&\,\mathrel{\ {{:}}}\,&&\IMode{\SCtx\times\Discrete{\Arities}\ra\Sets}\\
...
$\Of{\varrho}{\Upsilon\ra\Upsilon'}$, and $\IsAbt{\Omega}{\Upsilon}{\Gamma}{M}{\tau}$
...
$\Of{\circledast}{\int_{\Tuple{\tau, (\Upsilon\parallel\Gamma), (\Upsilon'\parallel\Gamma')}}{{{P}_{\tau}(\Upsilon'\parallel\Gamma')}^{{V}_{\tau}(\Upsilon\parallel\Gamma)} \ra {{P}_{\tau}(\Upsilon'\parallel\Gamma')}^{{P}_{\tau}(\Upsilon\parallel\Gamma)}}}$.
\end{remark}
...
\[%
  \Of{\SemBrackets{\Omega\triangleright\Upsilon\parallel\Gamma\vdash%
      M:\tau}_P}{\SemBrackets{\Omega\triangleright\Upsilon\parallel\Gamma}_P\ra P_\tau}%
\]
