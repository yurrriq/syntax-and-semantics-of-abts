...
the meta-level. It is certainly not intended that we should be able to take a
context $\Match{H}{x:A}$ and apply the substitution $\IsSubst{M}{x}{H}{M:A}$,
which is nonsensical; on the other hand, if $x$ had been a \emph{symbol}, all
we could do is rename it in the context, since substitution is not defined for
symbols.
...
$\IsAbt{\Omega}{\Upsilon}{\Gamma}{M}{s}$.

%% TODO: implement refinement judgment for wellscopedness

For the sake of this example, we will just stipulate that anything of sort $\SortExp$ or $\SortType$ is
grammatical:
\begin{align*}
  \infer{
  ...
  \infer{
    \IsWf{\Omega}{\Upsilon}{\Gamma}{M}{\SortExp}
  }{
  }
\end{align*}
