...
\title{Syntax and Semantics of Indexed Abstract Binding Trees}
\date{}
...
arities.  Originally inspired by Martin-L\"of's theory of arities
\cite{martin-lof:1984}, unisorted abts have been deployed successfully as the
uniform syntactic framework for several implementations of Constructive Type
Theory, including Nuprl \cite{constable:1986}, MetaPRL \cite{hickey:2003} and
...
operators. This generalization to \emph{indexed abts} is essential for a
...
simply sorted second order algebraic theories treated by Fiore et al.\ are
equivalent to abstract binding trees augmented with a notion of second-order
variable (metavariable).

The contribution of this paper is the development of the syntax and semantics
of \emph{indexed multi-sorted abts}, an extension of second order universal
algebra to support symbol-indexed families of operators. Additionally, we have
developed the categorical semantics for indexed abts formally in Constructive
Type Theory using the Agda proof assistant.
...
construction\footnote{In this case, $\mathbf{C}\Grothendieck{}{\Psi}$ represents
  the category of elements of a copresheaf $\Of{\Psi}{\mathbf{C}\to\Sets}$ but
  we keep the $\mathbf{C}$ implicit and simply refer to it as the Grothendieck
  construction. Alternatively, this construction can be understood as a coend
...
along the codomain Grothendieck bifibration $\pi_{cod}$ where
$\pi_{\Operators}$ is also a discrete Grothendieck opfibration:

\[
\begin{tikzcd}[ampersand replacement = \&, sep = large]
  {\OMode{\Grothendieck{}{\Operators}}}
  \arrow[rr,
    "{\IMode{\vartheta\in\Operators\Pair{\Upsilon}{a}\ \mapsto\
      \ArrId{\Operators\Pair{\Upsilon}{a}}}}"] \arrow[d, "\IMode{\pi_{\Operators}}" left]
  \& \& {\IMode{\Arr{\Sets}}}
    \arrow[d, "\IMode{\pi_{cod}}"] \\
  {\IMode{\SCtx\times\Discrete{\Arities}}} \arrow[rr, "\IMode{\Operators}" below] \&
  \& {\IMode{\Sets}} \\
\end{tikzcd}
\]
